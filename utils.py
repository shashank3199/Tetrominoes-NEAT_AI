"""
  utils.py              :   This file contains utility functions and class for the game.
...
from Tetris.global_variables import NUM_COLUMNS, NUM_ROWS, SCORING_VECTOR
...
def generate_piece_possibilities(init_matrix, model, piece_rotations, next_piece_rotations=None):
    ...
    1. Next_piece_rotations is not None :   The function is being called to generate possibilities
                                            of current_piece, so it generates all cases of current
                                            piece and also calls the same function again, but with -
                                            piece_rotations = next_piece_rotations; and
                                            next_piece_rotations = None;
                                            This is done to check for the best fitness that we can
                                            achieve with the next piece for each current_piece
                                            possiblilty and is returned as list of -

                                            [[ possible_rotation, possible_x_position,
                                               current_piece_fitness, best_fitness_achievable_with_next_piece]]

    2. Next_piece_rotations is None     :   In this case, the possibilities that are generated are
                                            for next_piece and the rotation, x_position corresponding
                                            to the best fitness are irrelevant and is used just as
                                            an insight to the next possible outcome. It enables the
                                            computer to use advantage that humans have while playing
                                            tetris, that is to know what the next piece could fetch.
    """
    # initialize list
...
def try_possible_moves(tetris, model):
    ...
    Note:   the number of shifts and number rotations have a '-' sign with them,
            because it is sorted in descending order, i.e.

            Example:            p[1]    |     tetris.current_piece.x    | - abs(diff)
                            ----------------------------------------------------------
                      (i)       3       |               5               |   - 2
                      (ii)      3       |               9               |   - 6

                     Thus, the descending sort will prefer case (i)
